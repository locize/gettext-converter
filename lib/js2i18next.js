...
import { UNSAFE_KEYS } from './shared.js'
...
const isUnsafeKey = (key) => UNSAFE_KEYS.indexOf(key) > -1
...
const emptyOrObject = (key, value, options) => {
  ...
  if (options.fallbackToMsgId && !value) {
    return {
      [key]: toArrayIfNeeded(key, options)
    }
  }
...
const getGettextValues = (value, locale, targetKey, options) => {
  ...
  const isPlural = !!value.msgid_plural || value.msgid_plural === ''
...
export default function (js, options = {}) {
  ...
  const json = {}
  // options.fuzzy === true changes the return shape to { resources, fuzzy }: the
  // resources as before plus the i18next keys (paths joined with '.') written
  // from "#, fuzzy" entries (skipUntranslated drops those entries before this, as before)
  const fuzzyKeys = options.fuzzy === true ? [] : null
    ...
    Object.keys(context).forEach((key) => {
      ...
      if (key.indexOf(separator) > -1) {
        ...
        if (keys.some(isUnsafeKey)) {
          // skip keys that could pollute Object.prototype
          delete context[key]
          return
        }
        keyPath = keys.slice(0, keys.length - 1)
        let x = 0
      ...
      if (fuzzyKeys && isFuzzy(context[key])) {
        Object.keys(newValues).forEach((k) => fuzzyKeys.push(keyPath.concat(k).join('.')))
      }
    })
  })
  if (fuzzyKeys) return { resources: json, fuzzy: fuzzyKeys }
  return json
}
